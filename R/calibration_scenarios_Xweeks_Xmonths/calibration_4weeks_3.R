...
output_folder <- "output/calibration/scenarios//4weeks_8weeks/"
...
my_func <-
  function(df,
    ...
    dates_list_train <-
      dates_list_ft[1:28] #First 28 days for training
    dates_list_cal <-
      dates_list_ft[29:length(dates_list_ft)] # Next 8 weeks
    ...
    calibration_scenario <-
      paste0(substr(min(df$date), 1, 10), " 4weeks_8weeks")
    df_weather_pm_train <-
...
res_2min_dates <- lapply(dates_list[111:140],
                         function(x)
