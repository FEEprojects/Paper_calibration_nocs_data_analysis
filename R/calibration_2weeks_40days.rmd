...
res %>%
  dplyr::filter(grepl("SPS", sensor)) %>%
  dplyr::filter(RMSE > 5) %>%
  dplyr::select(sensor, method) %>%
  group_by(sensor, method) %>%
  summarise(n())
...
p_sps <- res %>%
  ...
  dplyr::filter(type == "Full") %>%
  dplyr::filter(dataset_type == "cal") %>%
  ggplot(aes(x = method, y = expanded_uncertainty, colour = sensor)) +
  stat_boxplot(geom = 'errorbar') +
  geom_boxplot(
    fatten = 0.8,
    outlier.size = 0.8,
  ...
  custom_theme +
  xlab("Calibration method") +
  ylab(quickText("expanded_uncertainty (ug/m3)")) +
  labs(colour = "Sensors") +
  scale_y_continuous(labels = scaleFUN,
                     limits = quantile(res$expanded_uncertainty, c(0.1, 0.9), na.rm = T)) +
  theme(axis.text.x = element_text(
    angle = 90,
    hjust = 0.95,
    vjust = 0.2
...
p_sps_sum <- res %>%
  ...
  dplyr::filter(type == "Full") %>%
  dplyr::filter(dataset_type == "cal") %>%
  ggplot(aes(x = method, y = expanded_uncertainty)) +
  stat_boxplot(geom = 'errorbar') +
  geom_boxplot(
    fatten = 0.8,
    outlier.size = 0.8,
...
p_pms_sum <- res %>%
  ...
  dplyr::filter(type == "Full") %>%
  dplyr::filter(dataset_type == "cal") %>%
  ggplot(aes(x = reorder(method, expanded_uncertainty), y = expanded_uncertainty)) +
  stat_boxplot(geom = 'errorbar', width = 0.5, size = 0.1) +
  geom_boxplot(
    fatten = 0.8,
    outlier.size = 0.8,
...
# Statistical differences between the methods per sensor_type
...
res %>%
  dplyr::filter(type == "Full") %>%
  dplyr::filter(dataset_type == "cal") %>%
  group_by(grepl("SPS", sensor), method) %>%
  shapiro_test(RMSE)
...
The groups are all normal. The PMS combination does not all have homogeneous
variances.
...
stat.test_pms <- res  %>%
  ...
  # ungroup() %>%
  #group_by(method) %>%
  t_test(RMSE ~ method, var.equal = F, ref.group = "Initial") %>%
  add_significance()
...
p_pms <- res %>%
  ...
  dplyr::filter(type == "Full") %>%
  dplyr::filter(dataset_type == "cal") %>%
  ggplot(aes(x = sensor, y = RMSE, colour = method)) +
  stat_boxplot(geom = 'errorbar') +
  geom_boxplot(
    fatten = 0.8,
    outlier.size = 0.8,
...
#     stat.test_pms, label = "p.adj.signif", tip.length = 0.01,
...
p_pms_sum <- p_pms_sum + stat_pvalue_manual(
  stat.test_pms,
  label = "p.adj.signif",
  tip.length = 0.01,
  bracket.nudge.y = 0,
