...
dates_list <- c()
for (i in 1:(length(dates) - 84)) {
  dates_list <- append(dates_list, list(dates[i:(i + 83)]))
}
...
my_func <-
  function(df,
    ...
    dates_list_train <-
      dates_list_ft[c(1:14, (length(dates_list_ft) - 13):length(dates_list_ft))]
    dates_list_cal <-
      dates_list_ft[15:(length(dates_list_ft) - 14)]
    ...
    calibration_scenario <-
      paste0(substr(min(df$date), 1, 10), " 2weeks_2months_2weeks")
    df_weather_pm_train <-
...
res_2min_dates <- lapply(dates_list[121:160],
                         function(x)
                           dplyr::filter(df_2min_roll, cutDay %in% x)) %>%
  lapply(., function(x)
    my_func(x, sensor_field = sensor_field, reference_field = reference_field))
