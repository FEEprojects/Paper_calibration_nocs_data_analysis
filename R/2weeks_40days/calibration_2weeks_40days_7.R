...
output_folder <-
  "output/calibration/2weeks_40days/"
...
# Get all the possible subset of dates.
df_2min$cutDay <- cut(df_2min$date, breaks = "1 day")
dates <- unique(df_2min$cutDay)
dates_list <- c()
for (i in 1:(length(dates) - 54)) {
  dates_list <- append(dates_list, list(dates[i:(i + 54)]))
}
...
my_func <-
  function(df,
           ...
           reference_field = "PM2.5") {
    message("Start")
    df_train <- df[df$date <= min(df$date) + days(14), ]
    
    df_calibration <- df[df$date > min(df$date) + days(14), ]
    ...
    calibration_scenario <- paste0(substr(min(df$date), 1, 10), " Test")
    df_weather_pm_train <-
