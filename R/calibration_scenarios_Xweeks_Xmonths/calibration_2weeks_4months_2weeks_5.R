...
output_folder <-
  "output/calibration/scenarios/2weeks_4months_2weeks/"
...
dates_list <- c()
for (i in 1:(length(dates) - 140)) {
  dates_list <- append(dates_list, list(dates[i:(i + 139)]))
}
...
my_func <-
  function(df,
    ...
    calibration_scenario <-
      paste0(substr(min(df$date), 1, 10), " 2weeks_4months_2weeks")
    df_weather_pm_train <-
...
res_2min_dates <- lapply(dates_list[131:166],
                         function(x)
